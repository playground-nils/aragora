...
def _run(path: Path) -> dict[str, Any]:
    ...
    return {
        "date": stamp[:10],
        "attempted_issue_count": int(summary.get("attempted_issue_count", 0) or 0),
        ...
        "rescue_count": sum(1 for issue in payload.get("issues", []) if isinstance(issue, dict) and issue.get("had_rescue")),
    }
