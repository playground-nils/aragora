...
def test_inbox_command_and_coordination_routes_export_descriptive_metadata() -> None:
    expected_metadata = {
        ("get", "/api/v1/inbox/command"): ("Fetch inbox command center view", ["Inbox"]),
        ("get", "/api/v1/inbox/sender-profile"): ("Get inbox sender profile", ["Inbox"]),
        ("get", "/api/v1/inbox/daily-digest"): ("Get inbox daily digest", ["Inbox"]),
        ("get", "/api/v1/coordination/workspaces"): (
            "List coordination workspaces",
            ["Coordination"],
        ),
        ("delete", "/api/v1/coordination/workspaces/{workspace_id}"): (
        ...
        ("delete", "/api/v1/coordination/consent/{consent_id}"): (
    ...
    for (method, path), (summary, tags) in expected_metadata.items():
        operation = _operation(path, method)
