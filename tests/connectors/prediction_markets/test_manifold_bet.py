...
def _mkt(mid: str = "m1", *, liq: int | None = 2000) -> str:
    p: dict = {
        "id": mid, "slug": f"s-{mid}", "question": "Q?",
        "creatorUsername": "a", "createdTime": 1_700_000_000_000,
        "closeTime": 1_800_000_000_000, "resolution": None,
...
def test_per_market_cap(monkeypatch: pytest.MonkeyPatch) -> None:
    a = _adapter(
        ...
        monkeypatch, per_market_cap_mana=50, per_day_cap_mana=500,
    )
...
def test_per_day_cap(monkeypatch: pytest.MonkeyPatch) -> None:
    a = _adapter(
        {("GET", "market/m1"): (200, _mkt("m1", liq=9999)),
         ...
         ("POST", "bet"): (200, json.dumps({"id": "b"}))},
        monkeypatch, per_market_cap_mana=60, per_day_cap_mana=100,
    )
...
def test_liquidity_fraction_cap(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    a = _adapter({("GET", "market/s"): (200, _mkt("s", liq=100))}, monkeypatch,
                 per_market_cap_mana=1000, per_day_cap_mana=10_000)
...
def test_no_liquidity_skips_fraction_check(monkeypatch: pytest.MonkeyPatch) -> None:
    a = _adapter(
        ...
        monkeypatch, per_market_cap_mana=100, per_day_cap_mana=1000,
    )
...
def test_place_bet_returns_result(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert r == ManifoldBetResult(bet_id="bet_abc", market_id="mx", stake_mana=25,
                                  probability=0.75, outcome="YES")


def test_outcome_no_forwarded(monkeypatch: pytest.MonkeyPatch) -> None:
    a = _adapter({("GET", "market/my"): (200, _mkt("my")),
                  ("POST", "bet"): (200, json.dumps({"id": "b"}))}, monkeypatch)
...
def test_betid_alias_accepted(monkeypatch: pytest.MonkeyPatch) -> None:
    a = _adapter({("GET", "market/ma"): (200, _mkt("ma")),
                  ("POST", "bet"): (200, json.dumps({"betId": "alias_xyz"}))}, monkeypatch)
...
def test_counters_accumulate(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    a = ManifoldBetAdapter(http_client=client, api_key="k",
                           per_market_cap_mana=200, per_day_cap_mana=1000)
...
def test_http_error_raises(monkeypatch: pytest.MonkeyPatch) -> None:
    a = _adapter({("GET", "market/me"): (200, _mkt("me")),
                  ("POST", "bet"): (400, json.dumps({"error": "bad"}))}, monkeypatch)
...
def test_missing_bet_id_raises(monkeypatch: pytest.MonkeyPatch) -> None:
    a = _adapter({("GET", "market/mn"): (200, _mkt("mn")),
                  ("POST", "bet"): (200, json.dumps({"amount": 10}))}, monkeypatch)
...
def test_counters_unchanged_on_failure(monkeypatch: pytest.MonkeyPatch) -> None:
    a = _adapter({("GET", "market/mf"): (200, _mkt("mf")),
                  ("POST", "bet"): (500, "err")}, monkeypatch)
