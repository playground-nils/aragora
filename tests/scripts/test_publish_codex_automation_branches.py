...
def test_outbox_superseded_branches_accepts_direct_aragora_state_root_env(
    tmp_path: Path, monkeypatch: Any
) -> None:
    repo_root = tmp_path / "detached-worktree"
    state_root = tmp_path / "shared-state" / ".aragora"
    repo_root.mkdir()
        ...
        json.dumps(
            {
                ...
                "local_evidence": {
                    "branch": "codex/stronger",
                    "supersedes_branch": "codex/direct-aragora-root",
                },
            }
        ),
        encoding="utf-8",
    ...
    assert mod.outbox_superseded_branches(repo_root) == {"codex/direct-aragora-root"}
