...
def test_audit_accepts_staggered_writer_contracts(tmp_path: Path) -> None:
    ...
    prompt = (
        "Read memory, repair one branch, validate locally, "
        "run python3 scripts/agent_bridge.py operator-snapshot --json, then refresh outbox."
    )
    for automation_id, minute in mod.CORE_WRITERS.items():
        _write_automation(
            tmp_path,
...
def test_audit_warns_writer_missing_agent_bridge_snapshot(tmp_path: Path) -> None:
    import check_codex_desktop_automations as mod
    ...
    bridge_issues = [
        ...
        if issue["code"] == "missing_prompt_contract_agent_bridge_operator_snapshot"
    ]
    assert len(bridge_issues) == 4
    assert payload["summary"] == {"active_count": 4, "error_count": 0, "warning_count": 4}
