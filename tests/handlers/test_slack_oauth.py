...
class TestInstall:
    ...
    @pytest.mark.asyncio
    async def test_redirect_location_contains_state(self, handler, mock_state_store):
        result = await handler.handle("GET", "/api/integrations/slack/install", {}, {}, {}, None)
        location = result.headers.get("Location", "")
...
class TestCallback:
    ...
    @pytest.mark.asyncio
    async def test_callback_httpx_not_available(self, handler):
        """When httpx is not installed, return 503."""
        import sys
...
class TestAuditLogger:
    ...
    def test_returns_none_when_import_fails(self, handler_module, monkeypatch):
        handler_module._slack_oauth_audit = None
...
class TestHandleCallingConventions:
    ...
    @pytest.mark.asyncio
    async def test_registry_call_with_body(self, handler, mock_workspace_store, monkeypatch):
