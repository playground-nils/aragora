...
def _collect_route_map_methods(handler_cls: type) -> dict[str, tuple[list[str], bool]]:
    ...
    return _collect_explicit_route_methods(route_map)


def _collect_explicit_route_methods(
    route_entries: Iterable[Any],
) -> dict[str, tuple[list[str], bool]]:
    valid_methods = {"get", "post", "put", "patch", "delete", "head", "options"}
    methods_by_route: dict[str, set[str]] = {}
    for route in route_entries:
        raw_route = _extract_route_path(route)
        if not isinstance(raw_route, str) or " " not in raw_route:
            continue
        method, _ = raw_route.split(" ", 1)
        method = method.lower()
...
def _collect_autogenerated_paths() -> dict[str, tuple[list[str], bool]]:
    ...
    for handler_cls in handler_classes:
        ...
        explicit_route_methods = _collect_explicit_route_methods(getattr(handler_cls, "ROUTES", ()))
        route_map_methods = _collect_route_map_methods(handler_cls)
        for template, (methods, route_inferred) in explicit_route_methods.items():
            existing = route_map_methods.get(template)
            if existing is None:
                route_map_methods[template] = (methods, route_inferred)
                continue
            combined_methods = sorted(set(existing[0]) | set(methods))
            route_map_methods[template] = (combined_methods, existing[1] or route_inferred)
        for attr_name in ("ROUTES", "ROUTE_PREFIXES"):
            routes: Any = getattr(handler_cls, attr_name, None)
            if not routes:
                continue
