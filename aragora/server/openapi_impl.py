...
def _filter_unhandled_paths(paths: dict[str, Any]) -> dict[str, Any]:
    ...
    def _is_fully_autogenerated(spec: dict[str, Any]) -> bool:
        saw_operation = False
        for method, operation in spec.items():
            if method.lower() not in methods or not isinstance(operation, dict):
                continue
            saw_operation = True
            if operation.get("x-autogenerated") is not True:
                return False
    ...
    for spec_path, spec in paths.items():
        ...
        if not _is_fully_autogenerated(spec):
        ...
        has_curated_operation = any(
            isinstance(operation, dict) and operation.get("x-autogenerated") is not True
