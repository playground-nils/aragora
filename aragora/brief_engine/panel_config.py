"""Panel configuration loader and validator for the brief-engine executor.

This module owns the YAML-driven panel roster schema used by the
generic Protocol B executor in :mod:`aragora.brief_engine.protocol`.
Mode 3 PDB is the reference consumer; its concrete YAML lives at
``aragora/config/pdb_panel.yaml`` and is loaded via the PDB compat
layer. Future brief variants supply their own YAML but reuse the
schema defined here.

This module is a pure schema + validation layer: no HTTP, no worker
queues, no artifact writing.
...
- :class:`BriefBudgetConfig` — per-brief / per-day USD caps
- :class:`BriefPanelSlot` — one slot's metadata (review role, lens,
  family, candidate provider ids, ``required`` flag)
- :class:`BriefPanelDefinition` — which slots participate in findings,
  critique, and which one acts as synthesizer
- :class:`BriefPromptSet` — which prompt templates this panel uses
- :class:`BriefPanelConfig` — the validated top-level config

Functions:

- :func:`load_panel_config` — load and validate a config from disk
...
- :func:`panel_slots` — return the ordered :class:`BriefPanelSlot`
  objects for a specific panel (preserves the panel-level ordering
  required by the executor)

Validation posture
------------------

The loader raises :class:`BriefPanelConfigError` with an exact
``field_path`` attribute so callers can surface the failing key path
to operators without string-matching. Every rule in the Mode 3 spec's
...
class BriefPanelConfigError(ValueError):
...
class BriefBudgetConfig:
    ...
    in :mod:`aragora.brief_engine.protocol` does not dip into the
    reserve; it is surfaced to the caller for downstream policy layers.
    """
...
@dataclass(frozen=True, slots=True)
class BriefPromptSet:
    """Named prompt-template identifiers per phase.

    Resolved to actual prompt text at invocation time via a
    variant-specific prompts module (for Mode 3 PDB:
    :mod:`aragora.pdb.prompts`). This dataclass only carries the
    template identifiers so the panel config remains behavior-free.
    """
...
@dataclass(frozen=True, slots=True)
class BriefPanelConfig:
    """Top-level validated brief panel configuration."""

    version: int
    default_panel: str
    default_prompt_set: str
    budget: BriefBudgetConfig
    slots: Mapping[str, BriefPanelSlot]
    panels: Mapping[str, BriefPanelDefinition]
    prompt_sets: Mapping[str, BriefPromptSet]
...
def load_panel_config(path: Path) -> BriefPanelConfig:
    """Load and validate a panel config from disk.

    ``path`` is the filesystem location of the YAML file to parse. YAML
    is parsed via :func:`yaml.safe_load` (so no code execution, no
    custom tags). Variant-specific wrappers (e.g., Mode 3 PDB) default
    to their committed YAML when callers omit a path — this generic
    loader requires the path explicitly to avoid coupling the engine to
    any single variant's config location.
    """
    target = Path(path)
    try:
        raw_text = target.read_text(encoding="utf-8")
    except FileNotFoundError as exc:
        raise BriefPanelConfigError(
            "<path>",
    ...
    if raw is None:
        raise BriefPanelConfigError("<root>", "panel config file is empty")
...
def validate_panel_config(raw: Mapping[str, Any]) -> BriefPanelConfig:
    ...
    if not slots_raw:
        raise BriefPanelConfigError("slots", "slots map must not be empty")
    slots = {sid: _parse_slot(sid, cfg) for sid, cfg in slots_raw.items()}

    panels_raw = _require(raw, "panels", Mapping, path="panels")
    if not panels_raw:
        raise BriefPanelConfigError("panels", "panels map must not be empty")
    ...
    if not prompt_sets_raw:
        raise BriefPanelConfigError("prompt_sets", "prompt_sets map must not be empty")
...
def _parse_budget(raw: Mapping[str, Any]) -> BriefBudgetConfig:
...
def _parse_slot(slot_id: str, raw: Any) -> BriefPanelSlot:
...
def _parse_prompt_set(prompt_set_id: str, raw: Any) -> BriefPromptSet:
...
def panel_slots(config: BriefPanelConfig, panel_id: str) -> tuple[BriefPanelSlot, ...]:
...
def provider_slot_definitions(
    ...
    source of truth for candidate availability; brief-engine callers do
    not fork it.
    """
...
def _require(raw: Mapping[str, Any], key: str, kind: type, *, path: str) -> Any:
    ...
    if key not in raw:
        raise BriefPanelConfigError(path, f"required key {key!r} is missing")
    value = raw[key]
