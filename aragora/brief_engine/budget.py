"""Protocol B budget reservation and denial logic.
...
yields an explicit :data:`BriefBudgetStatus.BUDGET_EXCEEDED` decision
...
:class:`BriefBudgetLedger` captures the rolling daily spend the caller
is responsible for maintaining (the storage layer is one candidate
store; callers may also supply an in-memory ledger for tests).
"""
...
from aragora.brief_engine.panel_config import BriefBudgetConfig, BriefPanelSlot
...
so brief-engine estimates don't diverge from the metadata-heuristic
packet the UI already shows.
"""
...
class BriefBudgetStatus(str, Enum):
...
class BriefBudgetReservation:
    ...
    _ledger: "BriefBudgetLedger | None" = field(default=None, repr=False)
    ...
    def charge(self, usd: float) -> None:
        ...
        if self.finalized:
            raise RuntimeError(
                "cannot charge against a finalized brief budget reservation; "
...
class BriefBudgetLedger:
    ...
    default is sufficient for unit tests; production wiring layers a
    durable backing store (storage, Redis, or similar) on top.
    """
...
class SlotCostEstimator:
    ...
    every slot. Tests and production callers can inject bespoke
    estimators (e.g., one that consults a model-cost table). The
    executor only calls :meth:`estimate`; subclassing is NOT required.
    """

    def __init__(self, default_usd: float = DEFAULT_SLOT_COST_USD) -> None:
        self._default_usd = default_usd

    def estimate(self, slot: BriefPanelSlot) -> float:
        return self._default_usd


def estimate_slot_costs(
    slots: Sequence[BriefPanelSlot],
...
def per_brief_cap_usd(
    budget: BriefBudgetConfig, review_budget: ReviewBudget | None = None
) -> float:
    ...
    :attr:`BriefBudgetConfig.per_brief_usd` wins. This mirrors the
    "compose with, do not replace" posture of
    :mod:`aragora.review.policy`: the repo-level review budget can
    tighten the brief-engine default but not silently loosen it.
    """
...
def evaluate_budget(
    *,
    findings_slots: Sequence[BriefPanelSlot],
    synthesizer_slot: BriefPanelSlot,
    budget: BriefBudgetConfig,
    ledger: BriefBudgetLedger,
    estimator: SlotCostEstimator | None = None,
    ...
    Returns a :class:`BriefBudgetDecision` whose ``status`` is one of
    :data:`BriefBudgetStatus.ALLOWED`,
    :data:`BriefBudgetStatus.BUDGET_DEGRADED`, or
    :data:`BriefBudgetStatus.BUDGET_EXCEEDED`. The caller is expected
    to then reserve the decided total via :func:`reserve` (or directly
    construct a :class:`BriefBudgetReservation`) before running
    findings.
    ...
    if minimum_cost > per_brief or minimum_cost > per_day_remaining_before:
        ...
        return BriefBudgetDecision(
            status=BriefBudgetStatus.BUDGET_EXCEEDED,
    ...
    status = BriefBudgetStatus.ALLOWED if not dropped else BriefBudgetStatus.BUDGET_DEGRADED
    if dropped:
...
def reserve(
    ...
    :data:`BriefBudgetStatus.BUDGET_EXCEEDED` decision — such a decision
