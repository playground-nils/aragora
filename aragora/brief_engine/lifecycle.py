...
One authoritative state per ``(pr_number, head_sha)`` pair — or, for
non-PR brief variants, per whatever ``(subject, revision)`` tuple the
caller keys the storage on. This module is pure types: no filesystem
I/O, no logging, no external imports beyond the standard library. The
storage layer (:mod:`aragora.brief_engine.storage`) is the only place
that materializes state on disk.

States
------

``absent``   — no brief is known for this ``(subject, revision)`` pair.
...
``stale``    — a ``ready`` brief whose ``head_sha`` (or variant-specific
               revision token) no longer matches the current source of
               truth. Stale briefs are moved to ``invalidated/`` and the
               tuple ``(subject, new_revision)`` begins a fresh
               ``absent → queued → …`` cycle.
...
::
    ...
    ready   → stale     (revision advanced; artifact moved to
                         invalidated/)
    failed  → queued    (retry; storage layer enforces that the failed
                         record is cleared atomically before the queued
                         record lands)
    stale   → queued    (regenerate for the same ``(subject,
                         new_revision)`` — the new revision is
                         effectively a new lifecycle; this transition is
                         expressed in state-machine terms for symmetry
                         but should not occur in practice because
                         ``get_state`` keys on ``(subject, revision)``)
...
class BriefLifecycleState(str, enum.Enum):
    """Canonical lifecycle states for a brief.
