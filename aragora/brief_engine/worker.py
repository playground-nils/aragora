...
- **Cancel-safe.** :meth:`BriefGenerationWorker.cancel` triggers
  ...
  underlying ``run_brief_protocol_b`` call runs in a thread executor;
  we treat cancellation as best-effort while that call is in flight
  and record ``failed`` with ``failed_phase="cancelled"``.
...
- **Storage is truth.** On every transition the worker updates the
  :mod:`aragora.brief_engine.storage` lifecycle record. If the process
  dies mid-run the disk state still reflects where things stopped.

The worker is intentionally minimal — no retry, no scheduling. Future
variants layer retry and batch semantics on top.

Each submitted :class:`JobRequest` carries its own ``runner`` callable
— a zero-arg entry point produced by the variant-specific caller that
wraps :func:`aragora.brief_engine.protocol.run_brief_protocol_b` with
the right config + prompt renderer. This keeps the worker fully
variant-neutral.
"""
...
from aragora.brief_engine import storage
...
# A runner closure: receives the live :class:`ProviderInvoker` and
# returns a :class:`BriefExecutionResult`. Variant-specific callers
# construct one via
# ``functools.partial(run_brief_protocol_b, input, config=..., prompts=...)``
# (see the PDB wrapper for the canonical wiring).
BriefRunner = Callable[[ProviderInvoker], BriefExecutionResult]
...
class JobRequest:
    ...
    ``runner`` wraps the variant-specific executor entry point so the
    worker stays variant-neutral. When omitted, the worker falls back
    to the legacy Mode 3 PDB executor for back-compat.
    """
...
class BriefGenerationWorker:
    ...
    def __init__(self, *, max_concurrent: int = 5) -> None:
        ...
        self._tasks: dict[JobKey, asyncio.Task[BriefExecutionResult]] = {}
    ...
    def submit(self, request: JobRequest) -> "Future[BriefExecutionResult]":
        ...
        def _schedule() -> asyncio.Task[BriefExecutionResult]:
        ...
        sched_future: Future[asyncio.Task[BriefExecutionResult]] = Future()
    ...
    def cancel(self, key: JobKey) -> bool:
        ...
        ``run_brief_protocol_b`` on the executor thread the actual work
        may continue until it returns; the storage layer records the
    ...
    async def _run_job(self, request: JobRequest) -> BriefExecutionResult:
        sem = self._semaphore
        if sem is None:
        ...
        try:
            async with sem:
                ...
                if state != BriefLifecycleState.QUEUED:
                    logger.warning(
                        "brief_engine.worker: job %s launched without queued state (saw %s)",
                        key,
                        state.value,
                    )
                ...
                runner = request.runner or _default_pdb_runner(request.input)

                # The runner is synchronous — hand it to the default
                # thread executor so Task.cancel can at least unblock
                # the coroutine even if the sync work keeps going.
                result: BriefExecutionResult = await asyncio.to_thread(runner, invoker)
            ...
            try:
                ...
                logger.exception(
                    "brief_engine.worker: failed to record cancel for pr=%s sha=%s",
                    key.pr_number,
                    key.head_sha,
                )
            raise
            ...
            logger.exception(
                "brief_engine.worker: run_brief_protocol_b failed for pr=%s sha=%s",
                key.pr_number,
                key.head_sha,
            )
            try:
                ...
                logger.exception(
                    "brief_engine.worker: failed to record exception state for pr=%s sha=%s",
                    key.pr_number,
                    key.head_sha,
                )
            return _empty_failed_result()
...
def _default_pdb_runner(input: BriefExecutionInput) -> BriefRunner:
    """Return a runner that invokes the Mode 3 PDB executor.

    Kept inside :mod:`aragora.brief_engine.worker` so Mode 3 callers
    that don't supply a ``runner`` on :class:`JobRequest` continue to
    work. Future variants pass their own runner and never touch this
    fallback.

    The dispatch is a dynamic attribute lookup on
    :mod:`aragora.pdb.worker` so existing Mode 3 tests that monkeypatch
    ``aragora.pdb.worker.run_protocol_b`` continue to observe their
    override — the module-level attribute is the same function as
    :func:`aragora.pdb.protocol.run_protocol_b`, re-exported for this
    reason.
    """

    def _run(invoker: ProviderInvoker) -> BriefExecutionResult:
        # Imported lazily to avoid a circular dependency with
        # :mod:`aragora.pdb` — the PDB package re-exports the generic
        # primitives at import time.
        from aragora.pdb import worker as pdb_worker_mod

        return pdb_worker_mod.run_protocol_b(input, invoker=invoker)

    return _run


async def _await_task(task: asyncio.Task[BriefExecutionResult]) -> BriefExecutionResult:
    return await task


async def _finalize_result(key: JobKey, result: BriefExecutionResult) -> None:
    """Persist the terminal lifecycle state for a completed job."""

    cost = float(result.actual_cost_usd)

    if result.status == BriefExecutionStatus.SUCCESS:
    ...
    if result.status == BriefExecutionStatus.DEGRADED:
    ...
    if result.status == BriefExecutionStatus.BUDGET_EXCEEDED:
        await asyncio.to_thread(
            storage.mark_failed,
            key.pr_number,
            key.head_sha,
...
def _empty_failed_result() -> BriefExecutionResult:
    """Return a minimal failed :class:`BriefExecutionResult` for worker errors."""
    from aragora.brief_engine.budget import BriefBudgetDecision, BriefBudgetStatus
    ...
    return BriefExecutionResult(
        ...
        budget_decision=BriefBudgetDecision(
            status=BriefBudgetStatus.ALLOWED,
